...
CREATE TABLE IF NOT EXISTS habit_master_habits (
  ...
  cue_description TEXT, -- Make it obvious
  craving_description TEXT, -- Make it attractive
  response_description TEXT, -- Make it easy
  reward_description TEXT, -- Make it satisfying
  
  -- Implementation Intentions (If-Then Planning)
  if_then_plan TEXT, -- "If [cue], then I will [action]"
  
  -- ACT Framework
  personal_value TEXT, -- What value does this habit align with?
  committed_action TEXT, -- What committed action will you take?
  
  -- Keystone Habit
  is_keystone BOOLEAN DEFAULT FALSE,
  keystone_impact TEXT, -- How this habit affects other areas
  
  -- CBT Framework
  automatic_thought TEXT, -- What thought triggers this habit?
  cognitive_distortion TEXT, -- What cognitive distortion is at play?
  reframe_statement TEXT, -- Healthy reframe
  ...
  -- SDT Framework
  autonomy_score INTEGER CHECK (autonomy_score >= 1 AND autonomy_score <= 10), -- Self-chosen
  competence_score INTEGER CHECK (competence_score >= 1 AND competence_score <= 10), -- Mastery
  relatedness_score INTEGER CHECK (relatedness_score >= 1 AND relatedness_score <= 10), -- Connection
...
CREATE TABLE IF NOT EXISTS habit_master_templates (
  ...
  category_id UUID REFERENCES habit_categories(id),
  habit_type VARCHAR(20) CHECK (habit_type IN ('positive', 'negative')) NOT NULL,
  framework VARCHAR(50), -- Which psychological framework this template is based on
