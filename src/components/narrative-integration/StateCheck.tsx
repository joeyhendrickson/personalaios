...
export default function StateCheck(props: {
  ...
  const isDone = useMemo(() => session.current_phase !== 'state_check', [session.current_phase])
    ...
    try {
      ...
      const res = await fetch(`/api/modules/narrative-integration/sessions/${session.id}`, {
        ...
        body: JSON.stringify({
          ...
          user_goal: goal || null,
          current_phase: ready ? 'event_inventory' : 'stabilization',
        }),
      })
    ...
    <div className="bg-white rounded-lg border border-gray-200 p-5">
      <div className="mb-4">
        ...
        <p className="text-sm text-gray-600">
          Short calibration first. Keep it high-level — no graphic details needed.
        </p>
      </div>
      ...
      <div className="mt-6 flex items-center justify-end">
        ...
        >
          {saving ? 'Saving…' : isDone ? 'Saved' : 'Continue'}
        </button>
      </div>
    </div>
  )
