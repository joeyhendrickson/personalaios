...
export async function PATCH(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    // Check if project should be marked as completed (100% progress)
    ...
    if (isCompleted && !project.is_completed) {
      ...
      message = `🎉 Congratulations! Project completed! Earned ${pointsEarned} points!`
    } else if (pointsEarned > 0) {
      message = `Earned ${pointsEarned} points!`
    } else if (pointsEarned < 0) {
      message = `Lost ${Math.abs(pointsEarned)} points.`
    } else {
