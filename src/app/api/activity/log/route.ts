...
function validateActivityData(body: unknown) {
  ...
  if (!bodyObj.activity_type || !validActivityTypes.includes(bodyObj.activity_type as string)) {
    throw new Error('Invalid activity_type')
  }

  return {
    activity_type: bodyObj.activity_type as string,
    activity_data: bodyObj.activity_data || null,
    page_url: (bodyObj.page_url as string) || null,
    session_id: (bodyObj.session_id as string) || null,
  }
}
