...
function estimateComplexity(title: string, description?: string): 'low' | 'medium' | 'high' {
  ...
  const highComplexityWords = [
    'build', 'create', 'develop', 'implement', 'design', 'analyze', 'research',
    'complete', 'finish', 'launch', 'deploy', 'integrate', 'optimize', 'refactor'
  ];
  
  // Low complexity indicators
  const lowComplexityWords = [
    'check', 'review', 'read', 'watch', 'call', 'email', 'send', 'update',
    'quick', 'simple', 'easy', 'basic', 'small', 'minor'
  ];
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (similarTasks.length >= 3) {
      ...
      const avgPoints = similarTasks.reduce((sum, task) => sum + task.points_value, 0) / similarTasks.length;
    ...
    return NextResponse.json({
      ...
      category_defaults: category ? CATEGORY_DEFAULTS[category as keyof typeof CATEGORY_DEFAULTS] : null,
    });
