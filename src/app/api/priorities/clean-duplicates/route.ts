...
// POST /api/priorities/clean-duplicates - Remove duplicate priorities
export async function POST(request: NextRequest) {
  try {
    ...
    const { data: priorities, error: fetchError } = await supabase
      .from('priorities')
      .select('id, title, priority_type, is_deleted')
      .eq('user_id', user.id)
      .eq('is_deleted', false)
    ...
    groupedPriorities.forEach((group) => {
      if (group.length > 1) {
    ...
    console.error('Unexpected error in clean-duplicates API:', error)
    return NextResponse.json(
      {
