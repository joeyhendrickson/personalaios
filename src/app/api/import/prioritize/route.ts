...
GOALS:
${goals.map((goal: ImportedGoal, index: number) => `
${index + 1}. ${goal.title}
...
TASKS:
${tasks.map((task: ImportedTask, index: number) => `
${index + 1}. ${task.title}
...
`;
        ...
        const validatedGoals = prioritizedData.goals?.map((goal: any) => ({
        ...
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        const validatedTasks = prioritizedData.tasks?.map((task: any) => ({
        ...
        return NextResponse.json({
          ...
          overallStrategy: prioritizedData.overallStrategy || 'Focus on high-priority goals and break them down into actionable tasks.',
          aiAnalysis: {
      ...
      return NextResponse.json({
        ...
        overallStrategy: 'Smart prioritization applied based on deadlines, impact points, and estimated time. Focus on high-impact goals with approaching deadlines first. Break large tasks into smaller chunks.',
        aiAnalysis: {
          ...
          urgentGoals: prioritizedGoals.filter((g: any) => {
