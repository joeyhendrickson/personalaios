...
export async function GET(request: NextRequest) {
  try {
    ...
    if (todayPointsError) {
      ...
      return NextResponse.json({ error: 'Database error', details: todayPointsError.message }, { status: 500 })
    }
    ...
    if (weeklyPointsError) {
      ...
      return NextResponse.json({ error: 'Database error', details: weeklyPointsError.message }, { status: 500 })
    }

    const weeklyPointsTotal = weeklyPoints?.reduce((sum, entry) => sum + (entry.points || 0), 0) || 0
    ...
    for (let i = 6; i >= 0; i--) {
      ...
      const dayPoints = weeklyPoints?.filter(entry => 
