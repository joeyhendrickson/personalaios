  ...
  try {
    ...
    if (goals.length === 0) {
      return NextResponse.json({ 
        message: 'No active goals found. Please create goals first to generate AI recommendations.',
        priorities: []
      }, { status: 200 });
    }
...
`;
    ...
    const completion = await openai.chat.completions.create({
      ...
      messages: [
        {
          role: "system",
          content: "You are a productivity expert who analyzes goals and creates actionable priorities. Always respond with valid JSON."
        },
        {
          role: "user",
          content: prompt
    ...
    if (validPriorities.length > 0) {
      const { error: insertError } = await supabase
        .from('priorities')
        .insert(validPriorities.map(priority => ({
          user_id: user.id,
          ...priority
        })));
