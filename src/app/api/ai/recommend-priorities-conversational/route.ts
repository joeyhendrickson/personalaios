...
export async function POST(request: NextRequest) {
  try {
    ...
    const { daily_intention, energy_level, time_available, focus_area } = conversationalPrioritySchema.parse(body);

    console.log('Generating conversational priorities for user:', user.id, 'with intention:', daily_intention);
    ...
    console.log('Data fetched - Goals:', goals.length, 'Projects:', projects.length, 'Tasks:', tasks.length);
    ...
    if (goals.length === 0 && projects.length === 0) {
      return NextResponse.json({ 
        message: 'No active goals or projects found. Please create goals or projects first to generate AI recommendations.',
        priorities: []
      }, { status: 200 });
    }
...
`;
    ...
    const { text: aiResponse } = await generateText({
      ...
      messages: [
        {
          role: "system",
          content: "You are a productivity expert who creates personalized daily priorities based on user intentions and goals. Always respond with valid JSON."
        },
        {
          role: "user",
          content: prompt
    ...
    const validPriorities = recommendedPriorities
      .filter(priority => priority.title && priority.description && priority.priority_score)
      .map((priority, index) => ({
    ...
    if (validPriorities.length > 0) {
      ...
      const prioritiesToInsert = validPriorities.map(priority => ({
        user_id: user.id,
        ...priority
      }));
    ...
    try {
      const firesResponse = await fetch(`${process.env.NEXTAUTH_URL || 'http://localhost:3000'}/api/priorities/sync-fires`, {
        method: 'POST',
        headers: {
