-- Create spiritual discipline trophy system
-- This creates a system for tracking habit achievements and awarding spiritual discipline trophies

-- Table for discipline trophies (the trophy definitions)
CREATE TABLE IF NOT EXISTS discipline_trophies (
  ...
  background_gradient VARCHAR(100), -- CSS gradient for trophy background
  sound_cue VARCHAR(50), -- Sound effect name
  created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
  updated_at TIMESTAMP WITH TIME ZONE DEFAULT NOW()
);

-- RLS for discipline_trophies (public read access)
...
-- Table for user's earned discipline trophies
CREATE TABLE IF NOT EXISTS user_discipline_trophies (
  ...
  created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
  updated_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
  UNIQUE(user_id, trophy_id, habit_id) -- Prevent duplicate trophies for same habit
);

-- RLS for user_discipline_trophies
...
-- Table for tracking habit completion counts (for trophy calculations)
CREATE TABLE IF NOT EXISTS habit_completion_counts (
  ...
  created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
  updated_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
  UNIQUE(user_id, habit_id) -- One record per user per habit
);

-- RLS for habit_completion_counts
...
-- Insert default discipline trophies
