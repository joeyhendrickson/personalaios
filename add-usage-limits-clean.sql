...
ALTER TABLE public.access_codes ADD COLUMN IF NOT EXISTS used_count INTEGER DEFAULT 0;

-- Update existing codes to have unlimited uses (NULL = unlimited)
UPDATE public.access_codes SET max_uses = NULL, used_count = 0 WHERE max_uses IS NULL;

-- Add constraint to ensure used_count doesn't exceed max_uses
ALTER TABLE public.access_codes ADD CONSTRAINT check_usage_limit CHECK (max_uses IS NULL OR used_count <= max_uses);
