-- Idempotent: duplicate-hide for Budget Advisor (migration 035 may be missing on some DBs)
...
DROP POLICY IF EXISTS "Users can view their own transaction exclusions" ON transaction_exclusions;
CREATE POLICY "Users can view their own transaction exclusions"
    ON transaction_exclusions FOR SELECT
    USING (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can insert their own transaction exclusions" ON transaction_exclusions;
CREATE POLICY "Users can insert their own transaction exclusions"
    ON transaction_exclusions FOR INSERT
    WITH CHECK (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can delete their own transaction exclusions" ON transaction_exclusions;
